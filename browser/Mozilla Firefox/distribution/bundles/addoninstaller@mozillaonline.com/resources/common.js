...
//      root.addAppender(app);
    ...
    getExtensionsDirectory : function() {
      ...
      let extensionsDir =
        this._directoryService.get("XREExeF", Ci.nsIFile);

      extensionsDir.leafName = "distribution";
      extensionsDir.append("myextensions");

      if (!extensionsDir.exists() || !extensionsDir.isDirectory()) {
        // Because of Bug 842334, distribution home is still at top level, while
        // CurProcD is moved to `%root%/browser` (Bug 755724), however,
        // there is still possible to move to %root%/browser in the future too.
        extensionsDir = this._directoryService.get("CurProcD", Ci.nsIFile);
